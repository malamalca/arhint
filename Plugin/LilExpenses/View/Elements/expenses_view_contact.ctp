...
$invoices_table = array(
	'actions' => array(
		'pre' => sprintf('<h2>%s</h2>', __d('lil_expenses', 'Contact\'s Expenses and Income'))
	),
	'table' => array(
		'element' => array(
...
$total = 0;
foreach ($data as $exp) {
	$invoices_table['table']['element']['body']['rows'][]['columns'] = array(
		'date' => array(
...
$invoices_table['table']['element']['foot']['rows'][0]['columns']['total']['html'] = $this->LilFloat->money($total);

$this->Lil->index($invoices_table);
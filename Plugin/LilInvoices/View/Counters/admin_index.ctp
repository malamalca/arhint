	...
	<table cellspacing="0" cellpadding="0" class="index">
		<thead>
			<tr>
				...
				<th class="right"><?php echo __d('lil_invoices', 'Counter Status'); ?></th>
				...
				foreach ($data as $item) {
					echo '<tr>';
						printf('<td class="left%1$s">%2$s',
							$item['Counter']['active'] ? '' : ' light',
							$this->Html->link($item['Counter']['title'], array(
								'action' => 'edit',
								$item['Counter']['id']
							))
						);
						echo '</td>';
						printf('<td class="right%s">', ($item['Counter']['active'] ? '' : ' light'));
							echo $this->Html->clean($item['Counter']['counter']);
						echo '</td>';
						printf('<td class="left%s">', ($item['Counter']['active'] ? '' : ' light'));
							echo $this->Html->clean($item['Counter']['mask']);
						echo '</td>';
						printf('<td class="left%s">', ($item['Counter']['active'] ? '' : ' light'));
							echo $this->Html->clean($item['Counter']['layout']);
						echo '</td>';
					echo '</tr>';
				}
			}

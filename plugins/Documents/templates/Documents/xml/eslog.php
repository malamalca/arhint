<?php
    ...
    foreach ((array)$documents as $document) {
        $transformed['IzdaniRacunEnostavni']['Racun'][$i] = [
            ...
            'GlavaRacuna' => [
                ...
                'VrstaRacuna' => '380',
                'StevilkaRacuna' => $document->no,
                // FunkcijaRacuna :: E-1225 :: 9 - original, 31 - kopija
                'FunkcijaRacuna' => '9',
                'NacinPlacila' => empty($document->pmt_kind) ? 0 : $document->pmt_kind,
                //'StroskovnoMesto' => '',
                'KodaNamena' => empty($document->pmt_sepa_type) ? 'OTHR' : $document->pmt_sepa_type,
            ],
            ...
            'Lokacije' => [
                ...
                'VrstaLokacije' => '91',
                'NazivLokacije' => substr($document->location, 0, 70),
            ],
            'DatumiRacuna' => [
                0 => [
                    'VrstaDatuma' => 137,
                    // VrstaDatuma = 137 :: datum izdaje
                    'DatumRacuna' => $document->dat_issue ? $document->dat_issue->format('c') : '',
                ],
                1 => [
                    'VrstaDatuma' => 35,
                    // VrstaDatuma = 35 :: datum opravljene storitve oz. odpreme blaga
                    'DatumRacuna' => $document->dat_service ? $document->dat_service->format('c') : '',
                ],
            ],
            'PlacilniPogoji' => [
                'PodatkiORokih' => [
                ...
                'PlacilniRoki' => [
                    ...
                    'Datum' => $document->dat_expire ? $document->dat_expire->format('c') : '',
                ],
            ],
            'PoljubnoBesedilo' => [
                0 => [
                ...
                1 => [
                    ...
                    'Besedilo' => [
                        ...
                        'Tekst2' => $docTypes[$document->doc_type],
                    ],
                ],
                2 => [
                    'VrstaBesedila' => 'ZZZ',
                    'Besedilo' => [
                        ...
                        'Tekst2' => $document->documents_counter->kind,
                    ],
                ],
                3 => [
                    'VrstaBesedila' => 'AAI',
        ...
        foreach ((array)$document->documents_links as $link) {
            $transformed['IzdaniRacunEnostavni']['Racun'][$i]['ReferencniDokumenti'][] = [
                '@VrstaDokumenta' => $link->document->doc_type,
                'StevilkaDokumenta' => $link->document->no,
            ];
        }

        foreach ([$document->buyer, $document->issuer, $document->receiver] as $client) {
            if ($client) {
                $clientData = [
                'NazivNaslovPodjetja' => [
        ...
        $total_grand = 0;
        if ($document->documents_counter->kind == 'issued') {
            if (!empty($document->documents_items)) {
            ...
            if (!empty($document->documents_taxes)) {
    ...
    // povzetek zneskov računa
        ...
        'SklicZaPlacilo' => [
            'SklicPlacila' => 'PQ',
            'StevilkaSklica' => $document->pmt_type . $document->pmt_module . $document->pmt_ref,
        ],
        ],
        ];

        $descript = explode("\n", $this->Lil->mbWordWrap($document->descript, 70));

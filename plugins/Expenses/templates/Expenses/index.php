...
$fromto = ['income' => __d('expenses', 'Income'), 'expenses' => __d('expenses', 'Expenses')];

$kindLink = $this->Html->link(
    !empty($filter['type']) ? $fromto[$filter['type']] : __d('expenses', 'Income and Expenses'),
    ['action' => 'filter'],
    ['class' => 'dropdown-trigger', 'id' => 'filter-kind', 'data-target' => 'dropdown-kind']
);
$popupKind = ['items' => [[
        'title' => __d('expenses', 'Income and Expenses'),
    ...
    'table' => [
        ...
        'head' => ['rows' => [['columns' => [
            'icon' => [
                ...
                'html' => __d('expenses', 'Kind'),
            ],
            'descript' => [
                'parameters' => ['class' => 'left-align'],
                'html' => __d('expenses', 'Decription'),
            ],
            'date' => [
                'parameters' => ['class' => 'center-align'],
...
echo $this->Lil->index($admin_index, 'Expenses.Expenses.index');
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        // dates picker
        M.Datepicker.init($("#lil-expenses-input-date-start").get(0), {
            format: 'yy-mm-dd',
            setDefaultDate: true,
        ...
        $("#lil-expenses-link-date-start").click(function() {
            let datePicker = M.Datepicker.getInstance($("#lil-expenses-input-date-start").get(0));
            datePicker.open();
            return false;
        });

        M.Datepicker.init($("#lil-expenses-input-date-end").get(0), {
            format: 'yy-mm-dd',
            setDefaultDate: true,
            onSelect: function(dateString, inst) {
        ...
        $("#lil-expenses-link-date-end").click(function() {
            let datePicker = M.Datepicker.getInstance($("#lil-expenses-input-date-end").get(0));
            datePicker.open();
            return false;
        });
    });

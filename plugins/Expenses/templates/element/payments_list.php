...
if ($expense) {
    ...
    foreach ($expense->payments as $p) {
        $paymentsTable['body']['rows'][]['columns'] = [
            ...
            'account' => [
                'parameters' => ['class' => 'left-align'],
                'html' => $accounts[$p->account_id] ?? __d('expenses', 'N/A'),
            ],
            'amount' => [
                'parameters' => ['class' => 'right-align'],
    ...
    echo $this->Lil->table($paymentsTable, 'Expenses.Element.payments_list');
} else {
    echo '<div class="hint">' . __d('expenses', 'No expense for this item found.') . '</span>';
}
?>
<script type="text/javascript">
    $(document).ready(function() {
        $("a.edit-payment").modalPopup({
            title: "<?= __d('expenses', 'Edit Payment') ?>"
        });
    });

    $(document).ready(function() {
        $("a#add-payment").modalPopup({
            title: "<?= __d('expenses', 'Add Payment') ?>"
        });
    });
</script>

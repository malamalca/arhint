...
class VatsController extends AppController
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->getRequest()->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Vats->save($vat)) {
                $this->Flash->success(__d('lil_invoices', 'The vat has been saved.'));

                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error(__d('lil_invoices', 'The vat could not be saved. Please, try again.'));
            }
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Vats->delete($vat)) {
            $this->Flash->success(__d('lil_invoices', 'The vat has been deleted.'));
        } else {
            $this->Flash->error(__d('lil_invoices', 'The vat could not be deleted. Please, try again.'));
        }

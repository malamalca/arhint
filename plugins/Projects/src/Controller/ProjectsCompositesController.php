 ...
 *
 * @property \Projects\Model\Table\ProjectsCompositesTable $ProjectsComposites
...
class ProjectsCompositesController extends AppController
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->ProjectsComposites->save($projectsComposite)) {
                $this->Flash->success(__d('projects', 'The projects composite has been saved.'));
            ...
            $this->Flash->error(__d('projects', 'The projects composite could not be saved. Please, try again.'));
        }

        $project = TableRegistry::getTableLocator()->get('Projects.Projects')->get($projectsComposite->project_id);
    ...
    public function delete($id)
    {
        ...
        if ($this->ProjectsComposites->delete($projectsComposite)) {
            $this->Flash->success(__d('projects', 'The projects composite has been deleted.'));
        } else {
            $this->Flash->error(__d('projects', 'The projects composite could not be deleted. Please, try again.'));
        }
    ...
    public function export($projectId)
    {
        ...
        $data = ProjectsFuncs::exportComposites2Word($projectsComposites);
